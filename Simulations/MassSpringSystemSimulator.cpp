...
void MassSpringSystemSimulator::clampMassPointsToBox(Vec3 pMin, Vec3 pMax) {
	for (int i = 0; i < m_iCountMassPoints; i++) {
		...
		m_MassPoints[i].position.x = std::max(m_MassPoints[i].position.x, pMin.x);
		m_MassPoints[i].position.y = std::max(m_MassPoints[i].position.y, pMin.y);
		m_MassPoints[i].position.z = std::max(m_MassPoints[i].position.z, pMin.z);
		m_MassPoints[i].position.x = std::min(m_MassPoints[i].position.x, pMax.x);
		m_MassPoints[i].position.y = std::min(m_MassPoints[i].position.y, pMax.y);
		m_MassPoints[i].position.z = std::min(m_MassPoints[i].position.z, pMax.z);
	}
}

void MassSpringSystemSimulator::clampTmpMassPointsToBox(Vec3 pMin, Vec3 pMax) {
	// Does the same as clampMassPointsToBox just for midpoint tmp positions
	for (int i = 0; i < m_iCountMassPoints; i++) {
		// Note: maximize/minimize are bugged
		//m_MassPoints[i].position.maximize(pMin);
		//m_MassPoints[i].position.minimize(pMax);
...
void MassSpringSystemSimulator::simulateTimestep(float timeStep)
{
	...
	case MIDPOINT:
		...
		for (int i = 0; i < m_iCountSprings; i++) {
			...
			//int mp1_factor = 1 + mp1.isFixed; // TODO linearize this check
			//int mp2_factor = 1 + mp2.isFixed;
